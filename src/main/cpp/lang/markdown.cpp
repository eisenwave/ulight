...
/// Per CommonMark spec §4.2: an optional closing sequence of `#` characters
/// preceded by a space may appear at the end.
[[nodiscard]]
...
private:
    ...
    /// `prev_char` is the character preceding this region, used for `_`
    /// opener/closer rules.
    void consume_inline_content(std::size_t length, char8_t prev_char = 0)
    {
        ...
        while (index < end_pos && !eof()) {
            ...
            else if (c == u8'!' && avail >= 2 && remainder[1] == u8'[') {
    ...
    bool expect_code_span(std::size_t avail)
    {
        ...
        while (i < avail) {
            ...
            if (i - close_start == n) {
                ...
                emit_and_advance(n, Highlight_Type::string_delim);
                if (close_start - n > 0) {
                    emit_and_advance(close_start - n, Highlight_Type::text_code);
                }
                emit_and_advance(n, Highlight_Type::string_delim);
                return true;
            }
    ...
    bool expect_autolink(const std::size_t avail)
    {
        ...
        for (std::size_t j = 1; j < i; ++j) {
            if (remainder[j] == u8':' || remainder[j] == u8'@') {
                has_marker = true;
                break;
            }
        }
        ...
        emit_and_advance(1, Highlight_Type::symbol_formatting); // '<'
        emit_and_advance(i - 1, Highlight_Type::text_link); // URL/email
        emit_and_advance(1, Highlight_Type::symbol_formatting); // '>'
        return true;
    }
    ...
    bool expect_html_entity(std::size_t avail)
    {
        if (avail < 2 || remainder[0] != u8'&') {
            return false;
        }
        ...
        if (i < avail && remainder[i] == u8'#') {
            ++i;
            if (i < avail && (remainder[i] == u8'x' || remainder[i] == u8'X')) {
                // Hex numeric entity.
                ++i;
                const std::size_t start = i;
                while (i < avail && is_ascii_hex_digit(remainder[i])) {
                    ++i;
                }
                if (i == start || i >= avail || remainder[i] != u8';') {
                    return false;
                }
            }
            else {
                ...
                const std::size_t start = i;
                while (i < avail && is_ascii_digit(remainder[i])) {
                    ++i;
                }
                if (i == start || i >= avail || remainder[i] != u8';') {
                    return false;
            ...
            const std::size_t start = i;
            while (i < avail && is_ascii_alphanumeric(remainder[i])) {
                ++i;
            }
            if (i == start || i >= avail || remainder[i] != u8';') {
                return false;
        ...
        // Emit entire `&...;` including the semicolon.
        emit_and_advance(i + 1, Highlight_Type::string_escape);
        return true;
    }
